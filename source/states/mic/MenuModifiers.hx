...
class MenuModifiers extends MusicBeatState
{
	...
	function refreshModifiers():Void {
		...
		for (num => item in ModifierVariables.modifiers_name) {
			...
			menuItem.frames = Paths.getSparrowAtlas('Modifiers');
			menuItem.animation.addByPrefix('idle', item[num] + " Idle", 24, true);
			menuItem.animation.addByPrefix('select', item[num] + " Select", 24, true);
	...
	{
		checker.x -= 0.20/(ClientPrefs.data.framerate/60);
		checker.y -= 0.05/(ClientPrefs.data.framerate/60);
		...
		if (selectable && !goingBack && !substated) {
			...
			if (controls.UI_UP_P || controls.UI_DOWN_P) {
				FlxG.camera.y += (controls.UI_UP_P ? -10 : 10);
			}
