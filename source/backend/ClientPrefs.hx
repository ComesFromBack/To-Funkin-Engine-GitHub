...
@:structInit class SaveVariables {
	...
	public var startingAnim:Bool = false;
	public var styleEngine:Int = 2;
	public var usingFont:Int = 0;
	public var rgbFPS:Bool = false;

	//		  Visuals & UI / Display 		\\
	public var camZooms:Bool = true;
	public var comboOffset:Array<Int> = [0, 0, 0, 0, 0, 0, 0, 0];
	...
	//			Mic'd UP Style Only				\\
	public var RatingVisible:Bool = false;
	public var MissesVisible:Bool = true;
	public var ComboVisible:Bool = true;
	public var NPSVisible:Bool = true;
	public var timeBarUseIconColor:Bool = true;
	public var preloadSettingsTexture:Bool = true;

	//			Kade Style Only				\\
	public var oldHealthBarColor:Bool = false;
	public var showKadeSongTxt:Bool = false;
	public var playingKadeMenuMusic:Bool = false;
...
class ClientPrefs {
	...
	inline public static function getVersion():String {
		final Version:String = "0.11a";
		var PackType:String = null;
		#if(cpp || windows)
		PackType = "Microsoft Windows";
		#elseif android
		PackType = "Linux Android";
		#elseif linux
		PackType = "Linux"; // Sorry Linux,I forget you :(
		#elseif ios
		PackType = "Unix iOS";
		#elseif web
		PackType = "Website Browser";
		#elseif switch
		PackType = "Switch";
		#else
		#if lua
	...
	public static function saveSettings() {
		...
		variables.bind('Settings_Client', '${CoolUtil.getSavePath()}/data/Client');
		var modifiers:FlxSave = new FlxSave();
		modifiers.bind('Modifiers_Client', '${CoolUtil.getSavePath()}/data/Client');
		var addonsSave:FlxSave = new FlxSave();
		addonsSave.bind('Addons_Client', '${CoolUtil.getSavePath()}/data/Client');
		var gameplaySetting:FlxSave = new FlxSave();
		gameplaySetting.bind('Gameplay_Client', '${CoolUtil.getSavePath()}/data/Client');
		...
		if(gameplaySetting.data.map == null) gameplaySetting.data.map = new Map<String,Dynamic>();

		for(name => value in data.gameplaySettings)
			gameplaySetting.data.map.set(name, value);

		#if ACHIEVEMENTS_ALLOWED Achievements.save(); #end
		variables.flush();
		modifiers.flush();
		addonsSave.flush();
		gameplaySetting.flush();

		//Placing this in a separate save so that it can be manually deleted without removing your Score and stuff
		var save:FlxSave = new FlxSave();
		save.bind('Controls_Extends', CoolUtil.getSavePath()+"/data/Controls/");
	...
	public static function loadPrefs() {
		...
		if(gameplaySetting.data.map != null) {
			var savedMap:Map<String, Dynamic> = gameplaySetting.data.map;
			for(name => value in savedMap)
				data.gameplaySettings.set(name, value);
		} else trace("WARN: Please Check Gameplay Settings Save File!");
