...
class SpectralAnalyzerEx #if funkin.vis extends SpectralAnalyzer #end
{
	...
	var _buffer_:Array<Float> = [];
	function recycledInterleaved(data:lime.utils.UInt8Array, bitsPerSample:Int):Array<Float>
	{
		switch(bitsPerSample)
		{
			case 8:
				_buffer_.resize(data.length);
				for (i in 0...data.length)
					_buffer_[i] = data[i] / 128.0;

			case 16:
				_buffer_.resize(Std.int(data.length / 2));
				for (i in 0..._buffer_.length)
					_buffer_[i] = data.getInt16(i * 2) / 32767.0;

			case 24:
				_buffer_.resize(Std.int(data.length / 3));
				for (i in 0..._buffer_.length)
					_buffer_[i] = data.getInt24(i * 3) / 8388607.0;

			case 32:
				_buffer_.resize(Std.int(data.length / 4));
				for (i in 0..._buffer_.length)
					_buffer_[i] = data.getInt32(i * 4) / 2147483647.0;
