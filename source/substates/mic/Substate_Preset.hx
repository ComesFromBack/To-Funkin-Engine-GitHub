        ...
        if (selectable && !goingBack)
        {
            ...
            if (controls.ACCEPT)
            {
                switch (optionShit[curSelected])
                {
                    ...
                    case 'load':
                        goingBack = true;
                        substates.mic.Substate_PresetLoad.coming = "Modifiers";
        ...
        menuItems.forEach(function(spr:FlxSprite)
            {
                if (!goingBack)
                {
                    ...
                    if (spr.ID == curSelected)
                        spr.scale.set(FlxMath.lerp(spr.scale.x, 1.5, camLerp/(ClientPrefs.data.framerate/60)), FlxMath.lerp(spr.scale.y, 1.5, 0.4/(ClientPrefs.data.framerate/60)));
                }
                else
