...
class FunkinLua {
	...
	public function new(scriptName:String) {
		...
		Lua_helper.add_callback(lua, "loadJsonFile", function(lib:String) {
			var file = null;
			if(lib != null)
				file = Json.parse(Paths.getTextFromFile(lib));
			return file;
		});
		...
		/*Lua_helper.add_callback(lua, "getSongTime", function(mode:String) {
			var time:String = '';
			var timeTotal = FlxG.sound.music.time;
			var timeLeft = 0;
			var timeElapsed = 0;
			if(mode != null)
			{
				if(mode == 'Time Left')
				{
					timeLeft = timeTotal - Math.floor(songTime / 1000);
					time = '$timeLeft / $timeTotal';
				}
				else if(mode == 'Time Elapsed')
				{
					timeElapsed = Math.floor(songTime / 1000);
					time = '$timeElapsed / $timeTotal';
				}
			}
		...
		Lua_helper.add_callback(lua, "debugPrint", function(text:Dynamic = '', color:String = 'RED') PlayState.instance.addTextToDebug(text, CoolUtil.colorFromString(color)));
