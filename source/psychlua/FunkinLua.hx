...
class FunkinLua {
	...
	public function new(scriptName:String) {
		...
		var title:TitleCustom = TitleCustom.instance;
		if(game != null) game.luaArray.push(this);
		if(title != null) title.luaArray.push(this);
		...
		Lua_helper.add_callback(lua, "runTimer", function(tag:String, time:Float = 1, loops:Int = 1) {
			...
			variables.set(tag, new FlxTimer().start(time, function(tmr:FlxTimer)
			{
				...
				if(game != null) game.callOnLuas('onTimerCompleted', [originalTag, tmr.loops, tmr.loopsLeft]);
				if(title != null) title.callOnLuas('onTimerCompleted', [originalTag, tmr.loops, tmr.loopsLeft]);
		...
		// Lua_helper.add_callback(lua, "removeSpriteColor", function(tag:String, color:Int = 0) {
		// 	tag = tag.replace('.', '');
		// 	LuaUtils.destroyObject(tag);
		// 	var leSprite:ModchartSprite = backend.FlxSpriteExtra.removeSpriteColorLua();
		...
		Lua_helper.add_callback(lua, "getPixelColor", function(obj:String, x:Int, y:Int) {
			...
			if(split.length > 1) {
				...
				if(spr != null && spr.pixels != null) 
					ret = spr.pixels.getPixel32(x, y);
			return ret;
			// return FlxColor.BLACK;
		});
		Lua_helper.add_callback(lua, "setPixelColor", function(obj:String, x:Int, y:Int, color:Int) {
			var split:Array<String> = obj.split('.');
			var spr:FlxSprite = LuaUtils.getObjectDirectly(split[0]);
			if(split.length > 1) {
				spr = LuaUtils.getVarInArray(LuaUtils.getPropertyLoop(split), split[split.length-1]);
			...
			spr.pixels.setPixel32(x, y, color);
		});
