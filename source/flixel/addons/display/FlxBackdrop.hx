...
class FlxBackdrop extends FlxSprite {
	...
	/**
	 * Select a mode for move your FlxBackDrop object.
	 * Default is `SETTING`
	 * 
	 * @see flixel.addons.display.FlxBackDrop.ScrollMode;
	 */
	public var scrMode:ScrollMode = SETTING;
	...
	/**
	 * Scroll speed on X and Y.
	 */
	public var scroll:BackdropScroll = new BackdropScroll(0,0);

	/**
	 * @param FRAMERATE_DISPLAY Your display's using framerate.
	 */
	final FRAMERATE_DISPLAY:Int = FlxG.stage.application.window.displayMode.refreshRate;
	/**
	 * @param GAME_FRAMERATE    Game setting's framerate value. (Get from `backend.ClientPrefs.data.framerate`)
	 */
	final GAME_FRAMERATE:Int = ClientPrefs.data.framerate;
	/**
	 * @param RUNTIME_FRAMERATE Game runtime framerate. (Get from `debug.FPSCounter.currentFPS`)
	 */
	final RUNTIME_FRAMERATE:Int = debug.FPSCounter.currentFPS;
	...
	override function draw() {
		...
		if (alpha == 0 || _frame.type == FlxFrameType.EMPTY || scale.x <= 0 || scale.y <= 0)
			return;
    ...
    override function update(elapsed:Float):Void {
		if(scrMode == SETTING) {
			if(scroll.x!=0) this.x -= scroll.x/(GAME_FRAMERATE/FRAMERATE_DISPLAY);
			if(scroll.y!=0) this.y -= scroll.y/(GAME_FRAMERATE/FRAMERATE_DISPLAY);
		} else if(scrMode == RUNTIME) {
			if(scroll.x!=0) this.x -= scroll.x/(RUNTIME_FRAMERATE/FRAMERATE_DISPLAY);
			if(scroll.y!=0) this.y -= scroll.y/(RUNTIME_FRAMERATE/FRAMERATE_DISPLAY);
		}
...
enum ScrollMode {
	/**
	 * Move in -> `GAME SET FRAMERATE` / `DISPLAY'S SET FRAMERATE`.
	 */
	SETTING;

	/**
	 * Move in -> `GAME RUNTIME FRAMERATE` / `DISPLAY'S SET FRAMERATE`.
	 */
	RUNTIME;
}
...
class BackdropScroll {
	...
	inline public function set(x:Float=0,y:Float=0):Void {
		this.x = x;
		this.y = y;
	}
}